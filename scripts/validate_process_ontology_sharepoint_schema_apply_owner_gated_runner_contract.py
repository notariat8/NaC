...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not contract_md.is_file():
            errors.append("Markdown owner-gated runner contract was not written")

        json_payload = json.loads(contract_json.read_text(encoding="utf-8"))
        if json_payload != payload:
            errors.append("JSON owner-gated runner contract must match returned payload")

        combined = contract_json.read_text(encoding="utf-8") + "\n" + contract_md.read_text(encoding="utf-8")
        for marker in FORBIDDEN_MARKERS:
            if marker.lower() in combined.lower():
                errors.append(f"owner-gated runner contract must not contain marker: {marker}")

    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print("process ontology SharePoint schema apply owner-gated runner contract validation passed")
    return 0
