...
DEFAULT_RELEASE_GATE_MATTER_ACCESS_APPLY_POLICY_SMOKE_ARTIFACT = Path(
    "out/m365/teams-sharepoint/matter-access-apply-policy-smoke.redacted.json"
)
DEFAULT_MATTER_ACCESS_APPLY_SMOKE_OUTPUT = Path(
    "out/m365/teams-sharepoint/matter-access-apply-smoke.redacted.json"
)
DEFAULT_RELEASE_GATE_MATTER_ACCESS_APPLY_SMOKE_ARTIFACT = DEFAULT_MATTER_ACCESS_APPLY_SMOKE_OUTPUT
...
def build_parser() -> argparse.ArgumentParser:
    ...
    kg_business_case_type_azure_worm = kg_sub.add_parser(
        "business-case-type-azure-worm-readiness",
        help="Prüft den S6b-Azure-Blob-WORM-Adapter und Lock-Vertrag ausschließlich offline.",
    )
    kg_business_case_type_azure_worm.add_argument(
        "--format", choices=["text", "json"], default=argparse.SUPPRESS
    )
    ...
    kg_process_ontology_schema_apply_runner_dry_run_artifact = kg_sub.add_parser(
        "process-ontology-schema-apply-runner-dry-run-artifact",
        help="Schreibt einen redigierten Dry-Run-Nachweis für einen späteren Graph-REST-Schema-Apply.",
    )
    kg_process_ontology_schema_apply_runner_dry_run_artifact.add_argument(
        "--format", choices=["text", "json"], default=argparse.SUPPRESS
    )
    kg_process_ontology_schema_apply_runner_dry_run_artifact.add_argument("--output", type=Path, default=None)
    kg_process_ontology_schema_apply_runner_dry_run_artifact.add_argument(
        "--markdown-output",
        type=Path,
        default=None,
    ...
    kg_process_ontology_schema_apply_owner_gated_runner_contract = kg_sub.add_parser(
        "process-ontology-schema-apply-owner-gated-runner-contract",
        help="Schreibt einen Offline-Runner-Vertrag für einen späteren owner-gated Graph-REST-Schema-Apply.",
    )
    kg_process_ontology_schema_apply_owner_gated_runner_contract.add_argument(
        "--format", choices=["text", "json"], default=argparse.SUPPRESS
    )
    kg_process_ontology_schema_apply_owner_gated_runner_contract.add_argument("--artifact-root", type=Path, default=None)
    kg_process_ontology_schema_apply_owner_gated_runner_contract.add_argument("--output", type=Path, default=None)
    kg_process_ontology_schema_apply_owner_gated_runner_contract.add_argument("--markdown-output", type=Path, default=None)
    kg_process_ontology_schema_apply_owner_gated_runner_contract.add_argument(
        "--no-ensure-default-artifacts",
        action="store_true",
    )
    ...
    teams_sharepoint.add_argument(
        ...
        type=Path,
        help="Optionaler Pfad zum redigierten Matter-Access-Apply-Policy-Smoke-Artefakt.",
    )
    teams_sharepoint.add_argument(
        "--release-gate-matter-access-apply-smoke-artifact",
        type=Path,
        help="Optionaler Pfad zum redigierten owner-gated Matter-Access-Apply-Smoke-Artefakt.",
    )
    teams_sharepoint.add_argument(
...
def command_contracts(args: argparse.Namespace) -> int:
    ...
    if args.contracts_command == "validate":
        validators = [
            ...
            ("Codex Subagent Operating Gate", "validate_codex_subagent_operating_gate.py"),
    ...
    if mode == "release-gate":
        ...
        approvals["release_gate"] = {
            "approval_text": (
                ...
                "Matter-Access-Apply-Request-Plan, Matter-Access-Apply-Policy-Smoke, "
    ...
    if args.m365_command == "teams-sharepoint":
        ...
        if args.teams_sharepoint_command == "bff-azure-readiness":
            ...
            if args.format == "json":
                ...
                print(format_azure_bff_readiness(readiness).rstrip())
        ...
        if args.teams_sharepoint_command == "release-gate-evidence":
            ...
            evidence = build_release_gate_evidence(
                ...
                matter_access_apply_policy_smoke_artifact=args.release_gate_matter_access_apply_policy_smoke_artifact,
                matter_access_apply_smoke_artifact=args.release_gate_matter_access_apply_smoke_artifact,
...
def _run_m365_release_gate(repo_root: Path, args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    release_gate_matter_access_apply_policy_smoke_artifact = _resolve_m365_release_gate_path(
        repo_root,
        args.release_gate_matter_access_apply_policy_smoke_artifact,
        DEFAULT_RELEASE_GATE_MATTER_ACCESS_APPLY_POLICY_SMOKE_ARTIFACT,
    )
    ...
    release_gate_matter_access_apply_smoke_artifact = _resolve_m365_release_gate_path(
        repo_root,
        args.release_gate_matter_access_apply_smoke_artifact,
        release_gate_run_artifact_dir / DEFAULT_RELEASE_GATE_MATTER_ACCESS_APPLY_SMOKE_ARTIFACT.name,
    )
    ...
    steps = [
        ...
        (
            ...
            [
                ...
                correlation_id,
                "--matter-access-apply-policy-smoke-output",
                str(release_gate_matter_access_apply_policy_smoke_artifact),
                "--format",
                "json",
            ],
        ),
        ...
        (
            ...
            [
                ...
                str(release_gate_matter_access_apply_policy_smoke_artifact),
                "--release-gate-matter-access-apply-smoke-artifact",
                str(release_gate_matter_access_apply_smoke_artifact),
                "--release-gate-runtime-certificate-expiry-artifact",
                str(runtime_certificate_expiry_output),
                "--release-gate-runtime-env-bootstrap-artifact",
    ...
    retention_index = _retain_m365_release_gate_artifacts(
        ...
        artifacts={
            ...
            "matter_access_apply_policy_smoke": release_gate_matter_access_apply_policy_smoke_artifact,
            "matter_access_apply_smoke": release_gate_matter_access_apply_smoke_artifact,
    ...
    *,
    ...
    return {
        ...
        "summary": {
            ...
            "matter_access_apply_policy_smoke_status": summary.get("matter_access_apply_policy_smoke_status"),
