...
This plugin is for Handelsregister application preparation, not register data retrieval. For notary-side filing workflows, run `noc-bnotk-xnp` first and require the local XNP authentication gate before XNotar/register handoff work. Default to plan-preview, local execution, explicit applicant/notary approval, and evidence metadata. Do not submit applications, automate notary systems or retrieve register data unless a separate reviewed connector explicitly implements that action.
...
2. If notary-side, require `noc-bnotk-xnp` readiness before Handelsregister package work.
