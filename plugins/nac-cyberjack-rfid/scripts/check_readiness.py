...
PLUGIN_NAME = "nac-cyberjack-rfid"
...
MORRIS_PROBE_APP_NAME = "NaCReadinessProbe"
MORRIS_STATUS_OK = 0
...
def probe_windows_morris_loopback_api(http_get: MorrisHttpGet | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if listreaders_status == MORRIS_STATUS_NO_READER:
            ...
            message = "morris loopback API reached PC/SC and returned NaCard; the binding works without reading card data."
        elif reiner_count:
...
def next_required_action(checks: list[dict[str, Any]]) -> str:
    ...
    return "Proceed to nac-bnotk-xnp readiness testing with human approval for any PIN or login step."
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Local readiness check for the NaC cyberJack/RFID card gate."
    )
    parser.add_argument(
        "--manual-card-present",
        default="unknown",
