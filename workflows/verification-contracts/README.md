...
- [business-case-type-graph-write-composition-s4c.verification.json](business-case-type-graph-write-composition-s4c.verification.json): Executable S4c acceptance contract for Issue #698 covering local SQLite generation-CAS and crash/restart blocking, the injected Graph REST v1.0 transport and credential port, five-operation composition smoke, redaction and zero socket/DNS, external credential-store, live Graph and tenant-write counters; live identity, central durability and tenant execution remain owner-gated.
...
- [business-case-type-production-composition-s4g.verification.json](business-case-type-production-composition-s4g.verification.json): Executable S4g acceptance contract for Issue #708 covering distinct Entra identities, exact writer/BFF permissions, trusted-local SQLite separation, strict Azure Blob WORM REST behavior, redaction and zero network, credential or tenant activity during offline assessment.
