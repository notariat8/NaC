...
- [workflows/contracts/m365-bff-performance-acceptance.contract.json](m365-bff-performance-acceptance.contract.json):
  Endpointgebundener, konservativer Abnahmevertrag für exakt 500 synthetische
  Workbench-Reads mit tenantweiter SharePoint-Baseline, Request-Allowance und
  Resource-Unit-Allowance jeweils `NOT_CLAIMED`, app-weitem Azure-Monitor-Cap,
  final gesetztem Monitorfenster, Remote-Lease und aggregierter redigierter
  Evidence; Live-Ausführung und Provisionierung bleiben Owner-gated.
...
- [business-case-type-production-composition-s4g.contract.json](business-case-type-production-composition-s4g.contract.json): Offline-S4g-Kompositionsvertrag für Issue #708 mit strikt getrennten Provisioning-, Writer- und BFF-Identitäten, read-only Identitätsprüfung, exakt lokalem SQLite-Layout sowie create-only Azure-Blob-WORM-REST-Transport. Zentrale PostgreSQL-Evidence, Broker, Signatur-Anchor, durable Reconciliation, irreversibler WORM-Lock und Live-Aktivierung bleiben geblockt.
