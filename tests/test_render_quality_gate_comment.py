...
class RenderQualityGateCommentTests(unittest.TestCase):
    ...
    def test_comment_renders_build_status_and_kg_readiness(self) -> None:
        ...
        self.assertIn("`5/5` negative cases detected", comment)
        self.assertIn("`missing_reason`, `expired_delegation`, `workspace_scope_violation`, `missing_cleanup`, `audit_readback_missing`", comment)
        self.assertIn("fail-closed before Graph writes", comment)
