...
from nac_runtime.status_source import (  # noqa: E402
...
class NotarkammerRuntimeStatusSourceTests(unittest.TestCase):
    ...
    def test_first_matter_source_resolver_keeps_injected_source(self) -> None:
        injected = JsonRuntimeMetadataSource(lambda _object_key: {})
    ...
    def test_json_source_can_back_same_status_display_contract(self) -> None:
        ...
        display = build_first_matter_status_display_from_metadata_source(
            source=JsonRuntimeMetadataSource(read_json),
        )

        self.assertEqual(requested_keys, [DEFAULT_FIRST_MATTER_OBJECT_KEY])
    ...
    def test_json_source_rejects_invalid_reader_payload(self) -> None:
        source = JsonRuntimeMetadataSource(lambda _object_key: [])  # type: ignore[arg-type]

        with self.assertRaisesRegex(ValueError, "runtime_metadata_source_not_object"):
            source.load_first_matter_metadata()

    def test_runtime_metadata_row_reader_feeds_status_display(self) -> None:
        fixture = json.loads(FIXTURE.read_text(encoding="utf-8"))
        requested_keys: list[str] = []
        ...
        reader = RuntimeMetadataRowReader(fetch_row)
        display = build_first_matter_status_display_from_metadata_source(
            source=JsonRuntimeMetadataSource(reader),
        )
    ...
    def test_env_runtime_source_is_inactive_without_runtime_mode(self) -> None:
        source = build_first_matter_runtime_metadata_source_from_env({})

        self.assertIsNone(source)

    def test_env_runtime_source_uses_row_reader_and_configured_object_key(self) -> None:
        fixture = json.loads(FIXTURE.read_text(encoding="utf-8"))
        requested_keys: list[str] = []
        ...
        source = build_first_matter_runtime_metadata_source_from_env(
            {
                "NAC_FIRST_MATTER_RUNTIME_SOURCE": "json",
                "NAC_FIRST_MATTER_RUNTIME_OBJECT_KEY": "runtime/m365/notarkammer-first.json",
    ...
    def test_env_runtime_source_fails_closed_when_configured_mode_lacks_row_fetcher(self) -> None:
        source = build_first_matter_runtime_metadata_source_from_env(
            {"NAC_FIRST_MATTER_RUNTIME_SOURCE": "m365-sharepoint"}
        )
    ...
    def test_env_runtime_source_fails_closed_for_archived_atp_mode(self) -> None:
        source = build_first_matter_runtime_metadata_source_from_env(
            {
                "NAC_FIRST_MATTER_RUNTIME_SOURCE": "atp",
                "NAC_LEGACY_STORE": "archived",
            },
        ...
        self.assertIsInstance(source, UnavailableRuntimeMetadataSource)
        with self.assertRaisesRegex(RuntimeMetadataSourceUnavailable, "legacy_atp_runtime_source_archived"):
            source.load_first_matter_metadata()

    def test_runtime_metadata_row_reader_normalizes_bytes_and_mapping_payloads(self) -> None:
        fixture = json.loads(FIXTURE.read_text(encoding="utf-8"))
        cases = (
        ...
        for row in cases:
            with self.subTest(row_keys=sorted(row.keys())):
                reader = RuntimeMetadataRowReader(lambda _object_key, row=row: row)
                payload = reader(DEFAULT_FIRST_MATTER_OBJECT_KEY)
    ...
    def test_runtime_metadata_row_reader_fails_closed_for_missing_or_malformed_rows(self) -> None:
        error_cases = (
            (
        ...
        for fetch_row, error_type, message in error_cases:
            with self.subTest(message=message):
                reader = RuntimeMetadataRowReader(fetch_row)  # type: ignore[arg-type]

                with self.assertRaisesRegex(error_type, message):
                    reader(DEFAULT_FIRST_MATTER_OBJECT_KEY)

    def test_runtime_metadata_row_reader_rejects_mandate_secret_and_productive_payloads(self) -> None:
        fixture = json.loads(FIXTURE.read_text(encoding="utf-8"))
        cases = (
        ...
        for field, value, message in cases:
            with self.subTest(field=field):
                payload = dict(fixture)
                payload[field] = value
                reader = RuntimeMetadataRowReader(lambda _object_key, payload=payload: {"payload_json": payload})
