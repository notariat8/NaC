...
class NotarkammerDemoPreflightTests(unittest.TestCase):
    ...
    def test_preflight_checklist_has_exact_read_only_demo_checks(self) -> None:
        for content in read_preflight_docs():
            ...
            self.assertIn("python scripts/nac.py m365 teams-sharepoint privileged-plan", content)
            self.assertNotIn("python scripts/nac.py tenant apply-request", content)
            ...
            self.assertIn("M365", content)
            self.assertIn("SharePoint", content)
            self.assertIn("healthz", content.lower())
