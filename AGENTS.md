...
- Vor NaC-Releases über OCI DevOps, Functions, OCIR oder API Gateway ist die repo-lokale Release-Erinnerung [workflows/skills/nac-release-memory/SKILL.md](workflows/skills/nac-release-memory/SKILL.md) zu lesen. Sie ist der agentenlesbare Speicher für commitgebundene Releases, Time-Ledger-Nutzung, wiederholte Release-Reibung und den Umgang mit OCI-Timeouts.
- Für wiederholte Release-Arbeit gilt zusätzlich das Release-Lane Context Pack in [release-lane-context.dev.json](/home/ubuntu/src/oci-landing-zone/runbooks/release-lane-context.dev.json): Es enthält dev-only nicht-sensitive Release-Lane-OCIDs, Stack-Variablenschlüssel und Hotpath-Kommandos, damit keine broad `list` discovery als Standardpfad wiederholt wird.
