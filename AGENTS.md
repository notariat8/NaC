...
- Jede Funktion8-/Microsoft-Anmeldeanforderung muss ausdrücklich `ofunk@funktion8` oder `funktion8@funktion8` als gewünschten Benutzer nennen. Die Formulierung „Funktion8-Login“ ohne eindeutige Kontoangabe ist keine handlungsfähige Login-Anfrage und darf keinen Browser-, Broker- oder Gerätecodefluss starten.
...
- Graft ist der verbindliche Context-Layer für agentische Arbeit ([policies/graft-context-layer-policy.yaml](policies/graft-context-layer-policy.yaml)): Tier 1 (tree-sitter, kein LLM, kein Key) ist Pflicht. Vor nichttrivialer Arbeit läuft `graft build`, `graft check` steht im strict-Quality-Gate ([scripts/validate_graft_context_layer.py](scripts/validate_graft_context_layer.py)), und der pi-Pfad bindet den Skill [workflows/skills/graft-context/SKILL.md](workflows/skills/graft-context/SKILL.md) ein. Der lokale `graft`-Cache wird nicht committed. pi hat keinen MCP-Support; Graft läuft in pi über die CLI, nicht über einen MCP-Server.
...
- Die Syft-Ausnahme für beide SBOM-Formate gilt nur für das eingefrorene Windows-Binary-Bundle des Issue #748: jede Bundle-Datei wird separat attestiert, entdeckte Pakete werden abgeglichen und die Lizenz-/Dateizuordnung unabhängig geprüft. Sie ersetzt keine allgemeine Python-Komponenten-SBOM; ohne geprüften Lizenzkatalog und gesonderte Freigabe entsteht kein Paketkandidat.
