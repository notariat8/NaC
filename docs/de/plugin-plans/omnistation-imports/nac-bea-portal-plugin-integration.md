...
Dieses Dokument plant die Integration des beA-Portals und der beA-nahen Arbeitsablaeufe als lokales bzw. mandantenfaehiges NaC-Plugin. Ziel ist nicht, das besondere elektronische Anwaltspostfach technisch zu ersetzen oder unkontrolliert zu automatisieren. Ziel ist ein sicherer, auditierbarer Arbeitsrahmen, in dem NaC:

- beA-Voraussetzungen prueft,
- lokale beA-Client-Security-Ablaeufe kontrolliert begleitet,
- Versand-, Empfangs-, Signatur- und eEB-Vorgaenge als NaC-Prozesse dokumentiert,
- Nachweise minimiert speichert,
- Kanzlei- und Mandanten-Onboarding reproduzierbar macht.

beA bleibt das fuehrende System fuer Postfachzugang, Versand, Empfang, Signatur, Entschluesselung und beA-spezifische Rechte. NaC fuehrt Governance, Prozessstatus, Evidence, Freigabe und Audit.
...
- NaC-Evidence zu Nutzerbestaetigungen und beA-Aktionen erzeugen,
- Exportpakete aus beA in eine NaC-Akte uebernehmen, wenn der Nutzer den Export bewusst bereitstellt,
...
- NaC erzeugt lokale Entwurfs- und Pruefpakete.
...
Das Plugin kann als lokaler MCP-Server oder lokaler HTTP-Service umgesetzt werden. MCP ist fuer Codex/NaC-Prozesse naheliegend; HTTP erleichtert Desktop- und Browser-Integrationen.
...
- `bea.record_inbox_check`
  - protokolliert, dass ein Posteingangscheck durchgefuehrt wurde, ohne Inhalte an NaC zu uebertragen.
- `bea.get_evidence`
...
Wichtig: `executed_in_bea` bedeutet nicht, dass NaC die Aktion selbst ausgefuehrt hat. Es bedeutet, dass der Nutzer die Aktion in beA bestaetigt und ein minimierter Nachweis in NaC abgelegt wurde.
...
- PIN, Karten- und Token-Geheimnisse bleiben lokal und ausserhalb von NaC.
- beA Client Security bleibt fuehrend fuer lokale Sicherheitsfunktionen.
- NaC speichert standardmaessig nur Hashes, Prozessstatus, Attestierungen und Audit-Referenzen.
...
10. Produktivfreigabe nur ueber NaC-Review.
...
- Klaeren, ob NaC nur Companion oder auch KSW-Integrationspartner werden soll.
...
- Duerfen Dokumentinhalte in NaC gespeichert werden oder nur Hashes/Referenzen?
