...
**Goal:** NaC erzeugt prüfbare OCI-Identity-Apply-Requests ohne produktive OCI-Schreiboperationen.

**Architecture:** `src/nac_identity/oci_tenant.py` bleibt die zentrale pure-Python-Domänenschicht. Die CLI ruft diese Logik auf, der Vertrag und Validator prüfen Gates, Secret-Freiheit und die klare Trennung zwischen Apply-Readiness und späterer produktiver Ausführung.

**Tech Stack:** Python Standard Library, `unittest`, NaC CLI, Workflow-Verträge, strikter Quality Gate.
