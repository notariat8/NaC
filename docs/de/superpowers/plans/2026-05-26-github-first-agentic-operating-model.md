...
**Goal:** GitHub wird für NaC zur operativen Arbeits- und Fortschrittsoberfläche für nichttriviale agentische Änderungen, ohne die repo-basierten Policies, Checks und Nachweise zu ersetzen.

**Architecture:** Die Umsetzung besteht aus einem GitHub Organization Project, einem führenden Issue, einer Policy-Erweiterung, synchronisierten Agenten-/Regelspiegeln und strukturierten Issue-/PR-Templates. Die maschinenlesbare Policy bleibt führend; Tests und Validatoren verhindern, dass Spiegel oder Templates von der Policy abweichen.

**Tech Stack:** GitHub CLI `gh` 2.92.0, GitHub Projects, YAML/Markdown, Python `unittest`, bestehende NaC-Validatoren.
...
GitHub wird die operative Steuerungsoberfläche für nichttriviale agentische NaC-Arbeit.

## Scope

- Process Policy um `github_first_operating_model` erweitern.
- Agenten- und Regelspiegel synchronisieren.
- Issue-/PR-Templates um Project-, Delivery-Mode-, Risk-Gate- und Validierungsangaben erweitern.
- GitHub Organization Project `NaC Control Plane` als Fortschrittsoberfläche einrichten.

## Nichtziele

- Keine echten Mandatsdaten in GitHub.
- Keine Secrets, PINs, Tokens oder privaten Dokumentinhalte in Issues, PRs, Projects oder Kommentaren.
- Kein Umgehen von Review-, Secret-Scan- oder Quality-Gates.

## Akzeptanzkriterien

- `github_first_operating_model` ist maschinenlesbar in `policies/process-policy.yaml`.
- Tests prüfen Policy, Spiegel, Operations-Dokumente und Templates.
- `NaC Control Plane` existiert in `notariat8`.
- Diese Arbeit läuft über Branch und Draft PR.
- Required GitHub Checks sind grün.

## Risiko / Datenschutz / Secrets

Risk Gate: `Policy`

Die Änderung betrifft Governance-Oberflächen. Es werden keine Mandatsdaten oder Secrets verarbeitet.
