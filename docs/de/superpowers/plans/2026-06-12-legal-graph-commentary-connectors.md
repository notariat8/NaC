...
**Goal:** Einen ersten prüfbaren NaC-Rechtsgraphen für Erbrecht bauen und lizenzierte Kommentarquellen als getrennten MCP/API-Connector-Track absichern.

**Architecture:** Der erste Slice liefert Verträge, Validatoren, einen statischen Erbrechts-Graphen, eine Python-Runtime, eine Patch-/Review-Dry-run-Pipeline und eine zentrale `nac legal-graph`-CLI. Automatische Quellenläufe erzeugen nur Review-Patches; fachliche Wahrheit entsteht erst nach menschlichem Review und Merge.

**Tech Stack:** Python Standard Library, `unittest`, JSON-Verträge, bestehende NaC-CLI, `workflows/legal-graph/` als mandatsdatenfreier Graph-Artefaktbereich.

---

## File Structure

- Create `workflows/contracts/legal-graph.contract.json`: Vertrag für Graphschema, erlaubte Quellen, Reviewpflicht und No-Auto-Merge-Regeln.
- Create `workflows/contracts/legal-commentary-connectors.contract.json`: Vertrag für lizenzierte Kommentar-/Verlagsquellen über MCP/API ohne Volltext- oder Credential-Speicherung.
- Create `scripts/validate_legal_graph_contracts.py`: deterministischer Validator für beide neuen Verträge.
- Modify `src/nac_cli/cli.py`: `nac contracts validate` ruft den neuen Validator auf; `nac legal-graph ...` wird als zentrale Bedienkante ergänzt.
- Create `src/nac_legal_graph/__init__.py`: Modulmarker und Versionsexport.
- Create `src/nac_legal_graph/catalog.py`: lädt statische Legal-Graph-Domain-Dateien und erzeugt Status-/Review-Payloads.
- Create `src/nac_legal_graph/patches.py`: erzeugt Review-Patches aus Quellen-Diff-Fixtures ohne Graph-Merge.
- Create `workflows/legal-graph/domains/erbrecht.graph.json`: erster mandatsdatenfreier Erbrechts-Graph.
- Create `workflows/legal-graph/fixtures/erbrecht-source-update.json`: Fixture für Update-Dry-run und Patch-Test.
- Create `tests/test_legal_graph_contracts.py`: Vertragstests und Validatorausführung.
- Create `tests/test_legal_graph.py`: Loader-, Graph-, Patch- und No-Mandate-Data-Tests.
- Modify `tests/test_nac_cli.py`: CLI-Abdeckung für `nac legal-graph status`, `review` und `update-dry-run`.
- Modify `workflows/contracts/README.md`: neue Verträge verlinken.
- Modify `docs/de/cli.md` and `docs/en/cli.md`: neue Bedienkante dokumentieren.
- Modify `docs/de/pruefung-standardisierung-start.md` and `docs/en/pruefung-standardisierung-start.md`: Prüfpfad um Legal Graph und Kommentar-Connector-Grenze ergänzen.

---

### Task 1: Verträge und Validator bauen
...
### Task 2: Erbrechts-Graph und Loader bauen
...
- [ ] **Step 3: Add the Erbrechts graph fixture**
...
### Task 3: Patch-/Update-Dry-run bauen
...
### Task 4: `nac legal-graph` CLI ergänzen
...
### Task 5: Dokumentation, Index und Prüfpfad ergänzen
...
In the Fortschrittsbild row B, add one sentence:

```markdown
Der Legal-Graph-Track plant den ersten Erbrechts-MVP mit Primärquellen-Graph,
Review-Patches und getrenntem Kommentar-Connector-Vertrag für lizenzierte
MCP/API-Quellen ohne Volltext- oder Credential-Speicherung im Produktrepo.
```
...
- Spec coverage: M1 contracts, M2 Erbrecht-MVP, M3 update dry-run, CLI, docs, validation and commentary-connector boundaries are covered. M4 family/corporate expansion is intentionally not implemented in this first plan.
- Placeholder scan: keine Platzhaltermarker in den geplanten Implementierungsdateien; Tests nennen konkrete Schema-IDs und Guardrails.
