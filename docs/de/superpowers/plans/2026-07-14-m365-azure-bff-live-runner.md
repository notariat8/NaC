...
2. Eine vollständige read-only Inventur aller Entra-App-/Service-Principal-, UAMI-Rollen-, Site-Grant-, App-Catalog-, SPFx-Permission-/Install-, Seiten-/Webpart- und synthetischen Schlüssel-Treffer ausführen; Duplikate und breitere Rechte vor jedem möglichen Write ausschließen.
   Die von Application Insights automatisch erzeugte Action Group `Application Insights Smart Detection` wird nur mit der kanonischen ARM-ID der gebundenen Subscription und Resource Group akzeptiert. Ein exakt argumentgebundener Detail-Read über Azure Monitor API `2021-09-01` liegt zwischen zwei stabilen Inventar-Identitätsreads. Erforderlich sind `Global`, `SmartDetect`, aktiviert, exakt die ARM-Rollen `Monitoring Contributor` und `Monitoring Reader` mit Common Alert Schema sowie explizit leere Listen für alle anderen Receiver-Arten. Jede fremde oder veränderte Identität, Adapterausnahme, andere Action Group, fehlende Property oder Abweichung stoppt vor dem ersten Write.
