...
35. `xnotar_xjustiz_package_boundary` (nur `strict`)
36. `matter_data_classification_redaction` (nur `strict`)
37. `private_operating_frame_gate` (nur `strict`)
38. `private_payload_target_design` (nur `strict`)
39. `private_payload_access_policy` (nur `strict`)
40. `gnotkg_costs` (nur `strict`)
41. `secure_document_links` (nur `strict`)
42. `legal_research_connectors` (nur `strict`)
43. `legal_source_inventory_license_tdm` (nur `strict`)
44. `legal_model_customization_readiness` (nur `strict`)
45. `legal_model_card_ai_sbom_delta` (nur `strict`)
46. `legal_model_card_proposal` (nur `strict`)
47. `legal_ai_sbom_delta_proposal` (nur `strict`)
48. `legal_model_evaluation_benchmark` (nur `strict`)
49. `legal_graph_contracts` (nur `strict`)
