...
22. `process_ontology_sharepoint_schema_gap` (nur `strict`)
23. `process_ontology_sharepoint_schema_apply_plan` (nur `strict`)
24. `process_ontology_sharepoint_schema_apply_readiness` (nur `strict`)
25. `process_ontology_sharepoint_schema_apply_execution_contract` (nur `strict`)
26. `process_ontology_sharepoint_schema_apply_runner_dry_run` (nur `strict`)
27. `process_ontology_sharepoint_schema_apply_runner_dry_run_artifact` (nur `strict`)
28. `process_ontology_sharepoint_schema_apply_artifact_index` (nur `strict`)
29. `process_ontology_sharepoint_schema_apply_live_readiness_gate` (nur `strict`)
30. `process_ontology_sharepoint_schema_apply_owner_gated_live_plan` (nur `strict`)
31. `process_ontology_sharepoint_schema_apply_owner_gated_runner_contract` (nur `strict`)
32. `process_ontology_sharepoint_schema_apply_live_runner` (nur `strict`)
33. `process_ontology_sharepoint_schema_apply_graph_dispatcher` (nur `strict`)
34. `notarial_ontology_scale_budget` (nur `strict`)
35. `notarial_deep_process_candidate_routing` (nur `strict`)
36. `first_wave_bpmn_outline` (nur `strict`)
37. `first_wave_bpmn_outline_gap_review` (nur `strict`)
38. `first_wave_bpmn_outline_gap_review_artifact` (nur `strict`)
39. `first_wave_process_deep_model` (nur `strict`)
40. `codex_parallel_review` (nur `strict`)
41. `codex_subagent_operating_gate` (nur `strict`)
42. `codex_worktree_operating_model` (nur `strict`)
43. `codex_agent_context_operating_model` (nur `strict`)
44. `codex_agent_context_index_audit` (nur `strict`)
45. `codex_memory_hooks_operating_model` (nur `strict`)
46. `codex_command_rules_operating_model` (nur `strict`)
47. `codex_command_rules_adoption_smoke` (nur `strict`)
48. `codex_5h_batch_run_envelope` (nur `strict`)
49. `verification_contracts_domain_pilot` (nur `strict`)
50. `teams_sharepoint_graph_data_plane` (nur `strict`)
51. `microsoft_first_onprem_target_architecture` (nur `strict`)
52. `m365_release_readiness_gate` (nur `strict`)
53. `m365_sharepoint_bpmn_viewer_adapter` (nur `strict`)
54. `m365_matter_access_delegation` (nur `strict`)
55. `m365_matter_access_decision_replay` (nur `strict`)
56. `m365_matter_access_apply_live_smoke_release_lane` (nur `strict`)
57. `m365_matter_access_apply_live_smoke_retention` (nur `strict`)
58. `notarial_application_interface_inventory` (nur `strict`)
59. `xnotar_xjustiz_package_boundary` (nur `strict`)
60. `matter_data_classification_redaction` (nur `strict`)
61. `private_operating_frame_gate` (nur `strict`)
62. `private_payload_target_design` (nur `strict`)
63. `private_payload_access_policy` (nur `strict`)
64. `gnotkg_costs` (nur `strict`)
65. `secure_document_links` (nur `strict`)
66. `legal_research_connectors` (nur `strict`)
67. `legal_source_inventory_license_tdm` (nur `strict`)
68. `legal_model_customization_readiness` (nur `strict`)
69. `legal_model_card_ai_sbom_delta` (nur `strict`)
70. `legal_model_card_proposal` (nur `strict`)
71. `legal_ai_sbom_delta_proposal` (nur `strict`)
72. `legal_model_evaluation_benchmark` (nur `strict`)
73. `legal_graph_contracts` (nur `strict`)
