...
`bff-performance-infrastructure-owner-gate` erzeugt ausschließlich offline den hashgebundenen Owner-Kommentar für die dedizierte Koordinationsinfrastruktur und den vollständigen konservativen Messlauf. Der Befehl misst den sauberen Commit und Tree, die lokalen Toolchain-Attestierungen, das vollständige Quellpaket der Performance-Capability und -Runtime sowie die beiden angegebenen JSON-Eingaben. Er führt keine HTTP-, DNS-, Azure- oder Graph-Anfrage aus, erstellt oder ändert keine Ressource und startet keinen Messlauf. `READY` bedeutet nur, dass das Freigabeartefakt erzeugt wurde; es ist weder eine Live-Freigabe noch eine Provisioning- oder Ausführungsaktion.
