...
| P1 | M365/Onboarding | Public-Onboarding, DNS-Readiness, Request-Status und M365-/SharePoint-Plan sind als Vorführpfad dokumentiert; SharePoint ist das Store-Gate für den MVP. | Bewusster Fallback: Wenn Store oder Plan nicht verfügbar sind, wird `unavailable` als Einrichtungsstatus erklärt. | Keine OCI writes, keine Wallet-/DSN-Ausgabe, keine Secret-Prüfung im Termin, kein Provisioning jenseits von Review- oder Read-only-Nachweisen. | M365-/SharePoint-Plan mit nicht-sensitiver Statusprojektion stabilisieren und Onboarding-Request-Store für synthetische Demo-Anfragen verbindlich vorbereiten. |
