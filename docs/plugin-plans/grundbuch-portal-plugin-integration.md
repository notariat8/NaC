...
OaC8 soll Grundbuch-bezogene Vorgaenge fuer berechtigte SaaS-Kunden nachvollziehbar fuehren:
...
1. **Zulassungs- und Workflow-Companion**
   - OaC fuehrt Zulassungen, Rollen, berechtigtes Interesse, Aktenbezug, Vollmachten, Gebuehren und Evidence.
   - Der Abruf selbst erfolgt weiterhin durch berechtigte Nutzer im offiziellen Laenderportal.
   - OaC speichert standardmaessig keine Grundbuchinhalte, sondern Hashes und Nachweise.

2. **Evidence-Import und kontrollierte Ablage**
   - Nutzer koennen Grundbuchauszuege oder Abrufnachweise explizit bereitstellen.
   - OaC erfasst Hash, Abrufgrund, Aktenzeichen, Portal/Bundesland, Zeitstempel und Verantwortliche.
...
- keine Portal-Zugangsdaten, Kennwoerter, Zertifikate oder Codezeichen in OaC speichern
...
- OaC verwaltet Zulassungen und Rollen.
- OaC erzeugt Abrufvorgaenge mit Pflichtfeldern.
- OaC prueft, ob fuer Land, Nutzergruppe und Abrufgrund eine passende Zulassung dokumentiert ist.
- OaC oeffnet das offizielle Portal, fuehrt aber keinen Abruf aus.
...
- OaC speichert nur Hash, Speicherort, Aktenzeichen, Zeitstempel und Freigaben.
- Wenn Inhalte in OaC gespeichert werden, dann nur tenantisoliert, verschluesselt, mit strenger Retention und Zugriffskontrolle.
...
```json
{
  ...
  "legitimate_interest": {
    ...
    "approved_by": "oac-user-id"
  },
  "retrieval": {
    "performed_externally": true,
    "performed_by": "oac-user-id",
...
- Standard ist: keine Inhalte in OaC, nur Hashes und Metadaten.
...
1. Rolle klaeren: OaC8 als Prozessbegleiter, technischer Dienstleister oder selbst zugelassene Stelle.
...
4. Zulassungsnachweise und Genehmigungsumfang als OaC-Objekte modellieren.
...
10. Pilot mit rein manuellem Portalabruf und OaC-Evidence durchfuehren.
...
3. OaC erfasst Zulassungsart, Gueltigkeit, Nutzerkreis und erlaubte Abrufgruende.
4. Kunde definiert Freigaberegeln und Retention fuer Grundbuchnachweise.
5. OaC legt Kundentenant und Evidence Store an.
6. OaC aktiviert Grundbuch-Prozessvorlagen.
7. Erster Abruf wird manuell im offiziellen Portal durchgefuehrt.
8. OaC erfasst berechtigtes Interesse, Attestation, Hash und Gebuehrenereignis.
9. Review: Welche Daten duerfen spaeter gespeichert oder automatisiert verarbeitet werden?

## Offene Entscheidungen

- Soll OaC8 selbst jemals als zugelassene abrufende Stelle auftreten oder nur Kundenprozesse begleiten?
...
- Sollen Grundbuchauszuege ueberhaupt in OaC gespeichert werden oder nur im Kundensystem?
...
- Grundbuchinhalte werden standardmaessig nicht in OaC gespeichert.
