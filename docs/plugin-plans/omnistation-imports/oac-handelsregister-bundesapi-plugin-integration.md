...
1. **Register-Evidence-Companion**
   - OaC fuehrt den Businessprozess, die Recherche erfolgt manuell oder kontrolliert.
...
Kundenarbeitsplatz
  ...
  oac8-handelsregister-local-plugin
        |
...
- bei Bedarf Whitelist-IP-Antrag und Sonderfreigaben als OaC-Objekte dokumentieren
...
- OaC erzeugt einen Registerrecherche-Vorgang.
- Nutzer recherchiert im offiziellen Registerportal.
- OaC speichert Suchparameter, Zweck, Treffer-Metadaten, Hashes und Attestation.
- Keine automatisierten Portalabrufe.
- Keine Inhalte in OaC, solange der Kunde dies nicht explizit erlaubt.
...
Falls ein Adapter erlaubt und benoetigt ist, sollte OaC eine eigene kleine Komponente bauen:

- Request-Queue statt direkter Abrufe
- Token Bucket pro OaC-Installation, pro IP, pro Tenant und pro User
...
- Rate-Limit-Policy als OaC-Konfiguration
...
1. Rolle klaeren: OaC8 als Prozessbegleiter, technischer Rechercheadapter oder registrierter IP-Nutzer.
...
3. OaC legt Tenant, Quoten und Evidence Store an.
4. OaC aktiviert Prozessvorlagen fuer Registerrecherche.
5. Erste Recherche erfolgt manuell im offiziellen Registerportal.
6. OaC erfasst Suchzweck, Treffer-Metadaten, Attestation und Dokument-Hash.
...
- Soll OaC8 nur Recherche-Evidence dokumentieren oder selbst Abrufe ausloesen?
...
- Dokumente werden standardmaessig nicht in OaC gespeichert.
