...
3. For notary-side Online HRA work, install `oac-bnotk-xnp` before `oac-handelsregister`.
4. Install from the repo-local marketplace if supported by the Codex environment.
5. Confirm the installed plugin display name is `OaC Notary XNP Gate` and the source path is `./plugins/oac-bnotk-xnp`.
6. If the environment only supports home-local marketplaces, copy the reviewed plugin folders and marketplace entry after approval; keep the source of truth in this repository.
...
For Online HRA, `oac-bnotk-xnp` is only an installable readiness and authentication-gate companion. It does not authenticate as a notary by itself, store notary credentials, trigger XNotar imports or submit filings.
