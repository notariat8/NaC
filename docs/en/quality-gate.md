...
22. `process_ontology_sharepoint_schema_gap` only in `strict`
23. `process_ontology_sharepoint_schema_apply_plan` only in `strict`
24. `process_ontology_sharepoint_schema_apply_readiness` only in `strict`
25. `process_ontology_sharepoint_schema_apply_execution_contract` only in `strict`
26. `process_ontology_sharepoint_schema_apply_runner_dry_run` only in `strict`
27. `process_ontology_sharepoint_schema_apply_runner_dry_run_artifact` only in `strict`
28. `process_ontology_sharepoint_schema_apply_artifact_index` only in `strict`
29. `process_ontology_sharepoint_schema_apply_live_readiness_gate` only in `strict`
30. `process_ontology_sharepoint_schema_apply_owner_gated_live_plan` only in `strict`
31. `process_ontology_sharepoint_schema_apply_owner_gated_runner_contract` only in `strict`
32. `process_ontology_sharepoint_schema_apply_live_runner` only in `strict`
33. `process_ontology_sharepoint_schema_apply_graph_dispatcher` only in `strict`
34. `notarial_ontology_scale_budget` only in `strict`
35. `notarial_deep_process_candidate_routing` only in `strict`
36. `first_wave_bpmn_outline` only in `strict`
37. `first_wave_bpmn_outline_gap_review` only in `strict`
38. `first_wave_bpmn_outline_gap_review_artifact` only in `strict`
39. `first_wave_process_deep_model` only in `strict`
40. `codex_parallel_review` only in `strict`
41. `codex_subagent_operating_gate` only in `strict`
42. `codex_worktree_operating_model` only in `strict`
43. `codex_agent_context_operating_model` only in `strict`
44. `codex_agent_context_index_audit` only in `strict`
45. `codex_memory_hooks_operating_model` only in `strict`
46. `codex_command_rules_operating_model` only in `strict`
47. `codex_command_rules_adoption_smoke` only in `strict`
48. `codex_5h_batch_run_envelope` only in `strict`
49. `verification_contracts_domain_pilot` only in `strict`
50. `teams_sharepoint_graph_data_plane` only in `strict`
51. `microsoft_first_onprem_target_architecture` only in `strict`
52. `m365_release_readiness_gate` only in `strict`
53. `m365_sharepoint_bpmn_viewer_adapter` only in `strict`
54. `m365_matter_access_delegation` only in `strict`
55. `m365_matter_access_decision_replay` only in `strict`
56. `m365_matter_access_apply_live_smoke_release_lane` only in `strict`
57. `m365_matter_access_apply_live_smoke_retention` only in `strict`
58. `notarial_application_interface_inventory` only in `strict`
59. `xnotar_xjustiz_package_boundary` only in `strict`
60. `matter_data_classification_redaction` only in `strict`
61. `private_operating_frame_gate` only in `strict`
62. `private_payload_target_design` only in `strict`
63. `private_payload_access_policy` only in `strict`
64. `gnotkg_costs` only in `strict`
65. `secure_document_links` only in `strict`
66. `legal_research_connectors` only in `strict`
67. `legal_source_inventory_license_tdm` only in `strict`
68. `legal_model_customization_readiness` only in `strict`
69. `legal_model_card_ai_sbom_delta` only in `strict`
70. `legal_model_card_proposal` only in `strict`
71. `legal_ai_sbom_delta_proposal` only in `strict`
72. `legal_model_evaluation_benchmark` only in `strict`
73. `legal_graph_contracts` only in `strict`
