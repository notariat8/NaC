...
20. `agent_runtime_registry` only in `strict`
21. `agent_control_api` only in `strict`
22. `notarial_onprem_connector_boundaries` only in `strict`
...
37. `oci_tenant_identity` only in `strict`
