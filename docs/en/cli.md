...
`bff-performance-infrastructure-owner-gate` exclusively produces the hash-bound owner comment for the dedicated coordination infrastructure and complete conservative measurement run offline. It measures the clean commit and tree, local toolchain attestations, the complete performance capability/runtime source bundle, and the two supplied JSON inputs. It performs no HTTP, DNS, Azure, or Graph request, creates or changes no resource, and starts no measurement run. `READY` only means that the approval artifact was produced; it is neither live authorization nor a provisioning or execution action.
