...
attestation, the run stops before any provider port is created.
...
`bff-azure-activation-interruption-reconcile` is the separate edge for an externally interrupted run that has not reached a terminal state. Issue [#717](https://github.com/notariat8/NaC/issues/717) initially limits it to exact step 2 `ensure_resource_group`: step 1 is `PASSED`, step 2 is `RUNNING`, later steps are absent, the six ledger events form a valid hash chain, and the `target`, `legacy`, and `legacy_host` journals are `HELD` for the same activation hash. The default inspection supplies the complete persisted #632 live binding and reconciler binding to identify and verify the old run, but it is owner-free: it requires neither `--owner-approved` nor a new #717 approval. It performs two identical Azure read snapshots using `account show`, `provider show`, `group exists`, `group show`, and `resource list`; the allowed resource inventory is exactly empty. It mutates neither local artifacts nor provider state and emits only redacted binding and observation hashes with `MIDRUN_RECONCILIATION_REQUIRED`. A `Succeeded` resource group does not retroactively complete step 2 and does not permit resume. Invalid and changed observations return the actual stable codes `PROVIDER_OBSERVATION_INVALID` and `PROVIDER_OBSERVATION_DRIFT`.

The old #632 live approval remains only the binding of the interrupted run and is insufficient for the second operating mode. `--confirm-terminalize-and-release` requires a separate immutable owner comment in Issue #717. Its canonical body binds exactly `action`, `activation_hash`, `state_sha256`, `ledger_head_sha256`, `target_lock_sha256`, `legacy_lock_sha256`, `legacy_host_lock_sha256`, `provider_observation_sha256`, `interrupted_step`, `reconciler_commit`, `reconciler_tree`, `reconciler_toolchain_sha256`, and `required_owner_login`; the action is exactly `TERMINALIZE_AND_RELEASE_LOCK_ONLY`. After rechecking every precondition under an exclusive `flock`, the command may only terminalize the old run as `FAILED_PARTIAL`, finish step 2 with `EXTERNAL_PROCESS_INTERRUPTED_AFTER_WRITE`, write `MIDRUN_RELEASE_IN_PROGRESS`, and append `RELEASED` to all three journals. Resume, rollback, delete, `PASSED`, a success receipt, and provider writes remain forbidden. Unsupported state returns `INTERRUPTION_RECONCILIATION_UNSUPPORTED`; invalid or mismatched approval returns `INTERRUPTION_APPROVAL_INVALID` or `INTERRUPTION_APPROVAL_MISMATCH`; a torn release returns `INTERRUPTION_TERMINALIZATION_FAILED` and permits only a retry with the identical terminalization binding. A partial append may be repaired only when its bytes form a strict prefix of the deterministically expected `RELEASED` record; after runtime, state, path, and descriptor revalidation, only that tail is truncated to the owner-bound initial state. Any unknown tail blocks fail-closed.

Complete terminalization invocation (the first approval reference is the old #632 live binding; the terminalization approval reference is the new #717 decision):
...
Issue [#719](https://github.com/notariat8/NaC/issues/719) adds only the separate `BICEP_BASELINE_EXACT` classification; `RESOURCE_GROUP_ONLY` remains unchanged as legacy #717. The baseline expectation is derived from `prepared/prepared-inputs.redacted.json`, `prepared/main.json`, and `prepared/main.parameters.json` and bound to the activation hash, commit, tree, and prepared-manifest, template, parameter, and expectation hashes. Exactly seven top-level resources are accepted: Storage Account, Log Analytics Workspace, App Service Plan, User Assigned Managed Identity, Application Insights Component, Function App, and Smart Detection Action Group. The bound ARM deployment must be `Succeeded` and `Incremental` and, after fail-closed normalization, have exactly twelve unique successful deployment targets. Successful ARM sub-operations may repeat only the same normalized ID/type target; at most one successful targetless `EvaluateDeploymentOutput` metadata operation is allowed. Partial inventory, additional resources, and ID, type, status, region, tag, managed-identity, deployment, operation, or snapshot drift fail closed.

Each #719 snapshot reads account, providers, resource group, inventory, deployment, operations, the user-assigned managed identity, and the Function App identity assignment read-only; an exactly bound Azure Resource Graph POST additionally enumerates the resources and authorization resources visible there in the target scope. That set must consist of exactly the seven inventory resources and two role assignments. The four deployment child targets not enumerated by Resource Graph remain part of the twelve mandatory direct readbacks. Missing or additional visible resources or role assignments block with `AZURE_INTERRUPTION_RESOURCE_GRAPH_INVALID`. For all twelve directly read resources, the explicit allowlisted security-property projection must equal the Bicep target state exactly; additional read-only provider metadata outside that projection is allowed. Inventory is repeated within the snapshot, and the complete snapshot must be byte-identical across two observations. Inspection mutates no local artifacts and performs no Azure or tenant write. It accepts no terminal CLI fields; instead, it emits `provider_classification=BICEP_BASELINE_EXACT`, `baseline_expectation_sha256`, `prepared_inputs_manifest_sha256`, `bicep_snapshot_sha256`, and `bicep_parameters_snapshot_sha256` as part of the canonical #719 owner comment. The reconciler reads commit and tree with Git replace refs disabled and additionally binds its runtime files to the approved tree blob digests.
...
Separate #719 terminalization requires the unchanged hash values from that exact inspection, an immutable owner comment in Issue #719, and the unchanged action `TERMINALIZE_AND_RELEASE_LOCK_ONLY`. It finishes step 2 only as `FAILED` with `EXTERNAL_PROCESS_INTERRUPTED_AFTER_WRITE`, sets terminal run state `FAILED_PARTIAL`, and releases the three lock journals append-only. Resume, Azure or tenant writes, retry, rollback, delete, `PASSED`, and a success receipt remain forbidden.

Complete #719 terminalization invocation:
