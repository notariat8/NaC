...
| Knowledge graphs | `nac kg status`, `nac kg business-case-inventory`, `nac kg ontology-storage-contract`, `nac kg deep-process-candidates`, `nac kg workflow-contract <slug>` and `nac kg pilot-checklist <slug>` | Shows the state of usecase-local knowledge graphs, creates a thin business-case inventory for ontology sizing without a central knowledge graph, checks the ontology/storage boundary against SharePoint MVP and Graph REST rules, routes candidates for deep BPMN/ontology modeling, creates mandate-data-free workflow contract drafts and builds deterministic pilot intake checklists from a usecase KG. |
