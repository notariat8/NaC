...
7. Confirm the installed card plugin display name is `NoC Karte/SAK` and the source path is `./plugins/noc-cyberjack-rfid`.
8. Confirm the installed XNP plugin display name is `NoC XNP-Pruefung` and the source path is `./plugins/noc-bnotk-xnp`.
...
For Online HRA, `noc-cyberjack-rfid` is the installable `NoC Karte/SAK`, and `noc-bnotk-xnp` is the installable `NoC XNP-Pruefung`. They do not authenticate as a notary by themselves, store PINs or notary credentials, trigger XNotar imports or submit filings.
