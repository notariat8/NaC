...
**Goal:** NaC models the visible new-customer journey from `www-n8` to the initial customer admin in NaC.

**Architecture:** `www-n8` passes only domain hints. NaC handles readiness, DNS challenge, admin queue, apply plan and tenant registry. OCI starts with the Default Domain for SaaS admins, a secondary IAM domain for customer sign-in, one compartment per customer domain and shared ATP with tenant mapping.
...
def test_admin_queue_page_shows_customer_onboarding_request_without_secrets(self) -> None:
    ...
    self.assertIn("Readiness requests", html)
...
def test_customer_readiness_page_explains_next_steps(self) -> None:
    ...
    self.assertIn("Check DNS now", html)
    self.assertIn("return later", html)
    self.assertIn("No mandate data", html)
