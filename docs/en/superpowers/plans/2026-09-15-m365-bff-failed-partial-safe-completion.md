...
| `test_reconciler_binding_changes_require_new_approval_after_bounded_reads` | `reconciler_commit`, `reconciler_tree`, `reconciler_toolchain_sha256`, `required_owner_login`: two stable read-only snapshots, no current writes, new owner approval required | AC-746-03, AC-746-08 |
