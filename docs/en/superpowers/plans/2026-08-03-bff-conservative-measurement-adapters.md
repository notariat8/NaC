...
4. Implement a local BFF broker client and a server-side Function-system-identity-bound Azure Blob state machine with acquire, assert and release; the local runner receives neither a Storage token nor the lease ID (`AC-733-04`).
...
6. Bind measurement, monitor, broker lease, the `Performance.Lease` app role, Function settings, Function system identity, RBAC/ABAC, infrastructure plan and parameters, commit, tree, and toolchain into exactly one combined owner gate; prove the concrete lease and broker binding by readback (`AC-733-05`, `AC-733-08`).
