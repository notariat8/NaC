...
Status: Plan approved; offline builder repair in Draft PR, no package candidate, driver-release, or real-read approval
...
The verification contract expressly sets `repository_external_release_candidate`
to `false` for the current repair. A later package candidate requires its own
approval and a forward contract change.
