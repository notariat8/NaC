...
- [docs/en/architecture/process-ontology-sharepoint-schema-apply-runner-dry-run.md](architecture/process-ontology-sharepoint-schema-apply-runner-dry-run.md)
  ...
  artifact index, live-readiness gate, owner-gated live plan and runner
  contract before a later SharePoint schema apply over Microsoft Graph REST.
...
- [BusinessCaseType Graph Write Composition S4c](superpowers/specs/2026-07-29-business-case-type-graph-write-composition-s4c-design.md) implements Issue #698 offline: local SQLite generation-CAS/evidence state, an injected Graph REST v1.0 transport and synthetic token port compose all five S4b operations with crash/restart blocking; live Graph calls and tenant writes remain zero.
- [BusinessCaseType Live Write Boundary S4d](superpowers/specs/2026-07-29-business-case-type-live-write-boundary-s4d-design.md) composes Issue #700 offline: hash-bound owner attestation, separate write and BFF identities, local SQLite replay safety and complete S6/S6b publication before local closure; live adapters and tenant writes remain owner-gated.
