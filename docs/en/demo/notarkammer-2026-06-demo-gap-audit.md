...
| P1 | M365/Onboarding | Public onboarding, DNS readiness, request status and the M365/SharePoint plan are documented as a demo path; SharePoint is the MVP store gate. | Intentional Fallback: If the store or plan is unavailable, `unavailable` is explained as setup status. | No OCI writes, no wallet/DSN output, no secret inspection during the meeting, no provisioning beyond review or read-only evidence. | Stabilize the M365/SharePoint plan with a non-sensitive status projection and prepare the onboarding request store for synthetic demo requests. |
